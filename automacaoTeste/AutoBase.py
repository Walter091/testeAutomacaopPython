...
class utils():
    ...
    # ---------------------------------------------------------- S.O
    def toAreaDeTrabalho(self):
        py.hotkey("winleft", "d")

    def openWPSOfiice(self):
        self.toAreaDeTrabalho()
        py.press("winleft")
        time.sleep(2)
        py.write("WPS OFFICE")
        py.press("enter")
...
class planilhaUtils():
    ...
    def criarPlanilha(self, nomePlanilha):
        planilha = xl.Workbook()
        self.salvarPlanilha(planilha, nomePlanilha, False)
        print("Planilha Criada!")

    def carregarPlanilha(self, caminho):
        planilha = xl.load_workbook(caminho)
        print('planilha Carregada')
        return planilha

    def salvarPlanilha(self, planilha, nome, mostrarMsg):
        planilha.save(nome)
        if mostrarMsg:
            print("Planilha Salva!")

    def mesclarCelulas(self, planilha, celulas, linhaInicial, linhaFinal, colunaInicial, colunaFinal):
    ...
    def moverPlanilha(self, caminho, novoCaminho):
